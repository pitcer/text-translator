        ...
        if (!this.menu.isOpen) {
            ...
            clipboard.get_text(St.ClipboardType.CLIPBOARD, (clipboard, text) => {
                ...
                if (text_length < 1) {
                    ...
                    this._menu_open_clipboard.label.clutter_text.set_markup('Translate from clipboard(<span size="xx-small" color="grey"><i>empty</i></span>)');
                } else {
                    this._menu_open_clipboard.setSensitive(true);
                    this._menu_open_clipboard.label.clutter_text.set_markup('Translate from clipboard(<span size="xx-small" color="grey"><i>%s chars</i></span>)'.format(text_length));
                }

                clipboard.get_text(St.ClipboardType.PRIMARY, (clipboard, selection_text) => {
                    ...
                    if (selection_length < 1) {
                        ...
                        this._menu_open_selection.label.clutter_text.set_markup('Open with selection(<span size="xx-small" color="grey"><i>empty</i></span>)');
                    } else {
                        this._menu_open_selection.setSensitive(true);
                        this._menu_open_selection.label.clutter_text.set_markup('Open with selection(<span size="xx-small" color="grey"><i>%s chars</i></span>)'.format(selection_length));
                    }
                });
            });
        }
...
const TranslatorExtension = class TranslatorExtension {
    ...
    constructor() {
        ...
        this._dialog.source.clutter_text.connect('text-changed', () => {
            let enable_instant_translation = Utils.SETTINGS.get_boolean(PrefsKeys.INSTANT_TRANSLATION_KEY);
            if (!enable_instant_translation) {
                return;
            }
            ...
            if (TRIGGERS.translate) {
                TIMEOUT_IDS.instant_translation = Mainloop.timeout_add(INSTANT_TRANSLATION_DELAY, () => {
                    this._translate();
                });
            } else {
        ...
        Utils.SETTINGS.connect("changed::%s".format(PrefsKeys.SHOW_MOST_USED_KEY), () => {
            this._init_most_used();
        });
    }
    ...
    _show_most_used() {
        ...
        let most_used_sources = this._languages_stats.get_n_most_used(this._translators_manager.current.name, LanguagesStats.TYPE_SOURCE, 5);
        this._dialog.most_used.sources.set_languages(most_used_sources);

        let most_used_targets = this._languages_stats.get_n_most_used(this._translators_manager.current.name, LanguagesStats.TYPE_TARGET, 5);
    ...
    _init_languages_chooser() {
        this._source_language_chooser = new LanguageChooser.LanguageChooser('Choose source language:');
        this._source_language_chooser.connect('language-chose', (object, language) => {
            this._on_source_language_chose(object, language);
        });

        this._target_language_chooser = new LanguageChooser.LanguageChooser('Choose target language:');
    ...
    _on_key_press_event(object, event) {
        ...
        else if ((state == Clutter.ModifierType.SHIFT_MASK + Clutter.ModifierType.CONTROL_MASK || state == Clutter.ModifierType.SHIFT_MASK + cyrillic_control) && code == 54) {
            let text = this._dialog.target.text;

            if (Utils.is_blank(text)) {
                this._dialog.statusbar.add_message('There is nothing to copy.', 1500, StatusBar.MESSAGE_TYPES.error, false);
            } else {
                let clipboard = St.Clipboard.get_default();
                clipboard.set_text(St.ClipboardType.CLIPBOARD, text);
                this._dialog.statusbar.add_message('Translated text copied to clipboard.', 1500, StatusBar.MESSAGE_TYPES.info, false);
            }
        }
        // ctr+s - swap languages
        else if ((state == Clutter.ModifierType.CONTROL_MASK || state == cyrillic_control) && code == 39) {
            this._swap_languages();
        }
        // ctrl+d - reset languages to default
        else if ((state == Clutter.ModifierType.CONTROL_MASK || state == cyrillic_control) && code == 40) {
    ...
    _update_stats() {
        let source_data = {
            ...
            name: this._translators_manager.current.get_language_name(this._current_source_lang)
        };
        ...
        let target_data = {
            ...
            name: this._translators_manager.current.get_language_name(this._current_target_lang)
        };
        this._languages_stats.increment(this._translators_manager.current.name, LanguagesStats.TYPE_TARGET, target_data);
    }
    ...
    _current_langs_changed() {
        this._source_lang_button.label = 'from <i>%s</i>'.format(this._translators_manager.current.get_language_name(this._current_source_lang));
        this._target_lang_button.label = 'to <i>%s</i>'.format(this._translators_manager.current.get_language_name(this._current_target_lang));
    }
    ...
    _get_source_lang_button() {
        ...
        let label_text = '<u>From: %s</u>'.format(this._translators_manager.current.get_language_name(this._current_source_lang));
        let button = new ButtonsBar.ButtonsBarButton(false, label_text, 'Choose source language', button_params, () => {
            this._source_language_chooser.open();
            this._source_language_chooser.set_languages(this._translators_manager.current.get_languages());
            this._source_language_chooser.show_languages(this._current_source_lang);
        });
    ...
    _get_target_lang_button() {
        ...
        let label_text = '<u>To: %s</u>'.format(this._translators_manager.current.get_language_name(this._current_target_lang));
        let button = new ButtonsBar.ButtonsBarButton(false, label_text, 'Choose target language', button_params, () => {
            this._target_language_chooser.open();
            this._target_language_chooser.set_languages(this._translators_manager.current.get_pairs(this._current_source_lang));
            this._target_language_chooser.show_languages(this._current_target_lang);
        });
    ...
    _get_swap_langs_button() {
        ...
        let button = new ButtonsBar.ButtonsBarButton(false, ' \u21C4 ', 'Swap languages', button_params, () => {
            this._swap_languages();
        });
    ...
    _get_translators_button() {
        ...
        if (this._translators_manager.num_translators < 2) {
            button = new ButtonsBar.ButtonsBarLabel(this._translators_manager.current.name, 'tranlator-top-bar-button');
        } else {
            let button_params = {
                button_style_class: 'tranlator-top-bar-button-reactive',
                statusbar: this._dialog.statusbar
            };
            let label_text = '<u>%s</u>'.format(this._translators_manager.current.name);
            button = new ButtonsBar.ButtonsBarButton(false, label_text, 'Choose translation provider', button_params, () => {
    ...
    _get_translate_button() {
        ...
        let button = new ButtonsBar.ButtonsBarButton(false, 'Go!', 'Translate text(<Ctrl><Enter>)', button_params, () => {
            this._translate();
        });
    ...
    _get_instant_translation_button() {
        ...
        let button = new ButtonsBar.ButtonsBarButton(Utils.ICONS.instant_translation, '', 'Enable/Disable instant translation', button_params, () => {
            let checked = button.get_checked();
            button.set_checked(checked);

            Utils.SETTINGS.set_boolean(PrefsKeys.INSTANT_TRANSLATION_KEY, checked)
        });
        let checked = Utils.SETTINGS.get_boolean(PrefsKeys.INSTANT_TRANSLATION_KEY);
        button.set_checked(checked);
    ...
    _get_help_button() {
        ...
        let button = new ButtonsBar.ButtonsBarButton(Utils.ICONS.help, '', 'Help', button_params, () => {
            this._show_help();
        });
    ...
    _get_prefs_button() {
        ...
        let button = new ButtonsBar.ButtonsBarButton(Utils.ICONS.preferences, '', 'Preferences', button_params, () => {
            this.close();
            launch_extension_prefs(Me.uuid);
        });
    ...
    _get_close_button() {
        ...
        let button = new ButtonsBar.ButtonsBarButton(Utils.ICONS.shutdown, '', 'Quit', button_params, () => {
            this.close();
        });
    ...
    _add_topbar_buttons() {
        ...
        let by_label = new ButtonsBar.ButtonsBarLabel(' with ', 'tranlator-top-bar-button');
    ...
    _translate() {
        if (Utils.is_blank(this._dialog.source.text)) {
            return;
        }

        this._update_stats();
        this._dialog.target.text = '';
        let message_id = this._dialog.statusbar.add_message('Translating...', 0, StatusBar.MESSAGE_TYPES.info, true);

        this._translators_manager.current.translate(this._current_source_lang, this._current_target_lang, this._dialog.source.text, (result) => {
            this._dialog.statusbar.remove_message(message_id);

            if (result.error) {
                this._dialog.statusbar.add_message(result.message, 4000, StatusBar.MESSAGE_TYPES.error);
            } else {
                this._dialog.target.markup = result;

                if (Utils.SETTINGS.get_boolean(PrefsKeys.ENABLE_AUTO_SPEAK_KEY)) {
                    this._dialog.google_tts.speak(this._dialog.target.text, this._current_target_lang);
                }
            }
        });
    }
    ...
    _translate_from_clipboard(clipboard_type) {
        ...
        clipboard.get_text(clipboard_type, (clipboard, text) => {
            if (Utils.is_blank(text)) {
                this._dialog.statusbar.add_message('Clipboard is empty.', 2000, StatusBar.MESSAGE_TYPES.error, false);
                return;
            }
    ...
    _add_keybindings() {
        Main.wm.addKeybinding(PrefsKeys.OPEN_TRANSLATOR_KEY, Utils.SETTINGS, Meta.KeyBindingFlags.NONE, Shell.ActionMode.NORMAL | Shell.ActionMode.OVERVIEW, () => {
            this.open();
        });

        Main.wm.addKeybinding(PrefsKeys.TRANSLATE_FROM_CLIPBOARD_KEY, Utils.SETTINGS, Meta.KeyBindingFlags.NONE, Shell.ActionMode.NORMAL | Shell.ActionMode.OVERVIEW, () => {
            this._translate_from_clipboard(St.ClipboardType.CLIPBOARD);
        });

        Main.wm.addKeybinding(PrefsKeys.TRANSLATE_FROM_SELECTION_KEY, Utils.SETTINGS, Meta.KeyBindingFlags.NONE, Shell.ActionMode.NORMAL | Shell.ActionMode.OVERVIEW, () => {
    ...
    open() {
        if (Utils.SETTINGS.get_boolean(PrefsKeys.REMEMBER_LAST_TRANSLATOR_KEY)) {
            let translator = this._translators_manager.last_used ? this._translators_manager.last_used.name : this._translators_manager.default.name;
        ...
        this._dialog.open();
        this._dialog.source.clutter_text.set_selection(0, this._dialog.source.length);
    ...
    enable() {
        ...
        CONNECTION_IDS.show_icon = Utils.SETTINGS.connect('changed::' + PrefsKeys.SHOW_ICON_KEY, () => {
            let show = Utils.SETTINGS.get_boolean(PrefsKeys.SHOW_ICON_KEY);

            if (show && !this._panel_button) {
                this._add_panel_button();
            }
            if (!show) {
        ...
        CONNECTION_IDS.enable_shortcuts = Utils.SETTINGS.connect('changed::' + PrefsKeys.ENABLE_SHORTCUTS_KEY, () => {
            let enable = Utils.SETTINGS.get_boolean(PrefsKeys.ENABLE_SHORTCUTS_KEY);
